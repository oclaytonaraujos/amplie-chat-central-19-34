...
export function InstanceBulkActions({ 
  ...
  const handleBulkConnect = async () => {
    setLoading(true);
    const disconnectedInstances = selectedInstancesData.filter(
  ...
  const handleBulkConfigureWebhook = async () => {
    setLoading(true);
    const inactiveWebhookInstances = selectedInstancesData.filter(
      instance => instance.webhook_status !== 'ativo'
    );
        ...
        <CardContent>
          {selectedInstances.length > 0 ? (
              ...
              <div className="text-sm text-muted-foreground">
                ...
                {disconnectedCount > 0 && (
                  <span className="ml-2 text-amber-600">
                    • {disconnectedCount} desconectada(s)
                  </span>
                )}
                {inactiveWebhookCount > 0 && (
                  <span className="ml-2 text-red-600">
                    • {inactiveWebhookCount} com webhook inativo
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {disconnectedCount > 0 && (
                  ...
                  >
                    ...
                    Conectar ({disconnectedCount})
                  </Button>
                )}

                {inactiveWebhookCount > 0 && (
                  <Button
                    onClick={handleBulkConfigureWebhook}
                    disabled={loading}
                    size="sm"
                    variant="outline"
                    className="flex items-center gap-2"
                  >
                    ...
                    Config Webhook ({inactiveWebhookCount})
                  </Button>
                )}
            ...
            <p className="text-sm text-muted-foreground">
              Selecione instâncias para executar ações em lote
            </p>
          )}
        </CardContent>
      </Card>
