...
interface CreateEditDialogProps {
  ...
  isLoading: boolean;
  editingInstance?: { id: string; instance_name: string } | null;
}
...
export function WhatsAppConnectionManager() {
  ...
  const [editingInstance, setEditingInstance] = useState<{ id: string; instance_name: string } | null>(null);
    ...
    for (const instance of instances) {
      try {
        const response = await fetch(
          `${config.server_url}/instance/connectionState/${instance.instance_name}`,
          {
            method: 'GET',
        ...
        newStates[instance.instance_name] = 'error';
      }
    }
    
    setConnectionStates(newStates);
  }, [config, instances, connectionStates, selectedInstance, showQRModal, toast]);
  ...
  const handleEditInstance = (instance: any) => {
    setEditingInstance({ id: instance.id, instance_name: instance.instance_name });
  ...
  const handleDeleteInstance = async (instanceName: string) => {
    setOperationLoading(prev => ({ ...prev, [instanceName]: true }));
    ...
    try {
      ...
      setOperationLoading(prev => ({ ...prev, [instanceName]: false }));
    }
  };
  ...
  const handleRefreshQR = async (instanceName?: string) => {
    ...
    try {
      const response = await fetch(
        `${config.server_url}/instance/connect/${targetInstance}`,
        {
          method: 'GET',
          headers: { apikey: config.api_key }
        }
  ...
  const handleDisconnect = async (instanceName: string) => {
    ...
    try {
      ...
      if (success) {
        await loadInstances();
        setConnectionStates(prev => ({ ...prev, [instanceName]: 'disconnected' }));
      }
    } finally {
      setOperationLoading(prev => ({ ...prev, [instanceName]: false }));
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <QRCodeModal
        ...
        connectionName={selectedInstance}
        qrCode={currentQR}
