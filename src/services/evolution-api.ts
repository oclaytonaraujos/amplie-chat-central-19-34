...
export class EvolutionApiGlobalService {
  ...
  async fetchInstances() {
    try {
      const data = await this.makeRequest('/manager/findInstance');
      return data;
    } catch (error) {
  ...
  async createInstance(instanceName: string, webhookUrl?: string) {
    try {
      const data = await this.makeRequest('/manager/create', {
        method: 'POST',
        body: JSON.stringify({
          instanceName,
          token: this.apiKey,
  ...
  async deleteInstance(instanceName: string) {
    try {
      const data = await this.makeRequest(`/manager/delete/${instanceName}`, {
        method: 'DELETE'
      });
      return data;
    } catch (error) {
